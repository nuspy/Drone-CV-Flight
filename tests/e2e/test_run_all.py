...
def pipeline(out_dir):
    ...
    cfg = load_config(
        ...
        overrides={
            ...
            "harness": {"episodes": 2, "time_cap_s": 120.0, "kidnap_test": True},
        },
        root=ROOT,
    )
...
def test_reliability_gate(pipeline):
    """The actual CI quality gate: blind localization within the CI
    thresholds, autonomous reach working, verdict PASSED."""
    cfg, _tr, rc = pipeline
    payload = json.loads((cfg.report_dir / "report.json").read_text())
    acc = payload["accuracy"]
    reach = payload["reach"]
    assert acc["pos_err_h_p95_m"] < cfg.harness.pass_thresholds.p95_pos_err_m
    assert reach["success_rate"] >= cfg.harness.pass_thresholds.reach_success_rate
